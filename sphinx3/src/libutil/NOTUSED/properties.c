...
prop_parse(propset_t * _pset, int _argc, char **_argv, int _flags)
{
    ...
    for (i = 0; i < _argc; i += 2) {
        ...
        if (orig != NULL) {
            prop_new_entry(_argv[i], orig->type, _argv[i + 1], orig->desc,
                           &repl);
        }
        else {
            prop_new_entry(_argv[i], PTYPE_UNKNOWN, _argv[i + 1], NULL,
                           &repl);
        }
        if (repl == NULL) {
            goto prop_parse_cleanup;
        ...
        hash_enter(parsed_ht, _argv[i], /* some meaningless number */
                   12345);
    }
               ...
               prop_t ** _prop)
{
    ...
    if (type_bits != PTYPE_STRING &&
        ...
        type_bits != PTYPE_DOUBLE && type_bits != PTYPE_UNKNOWN) {
        return -1;
    }
