...
public class Node {
     ...
     */
    protected Node(SearchState word, int beginTime, int endTime) {
     ...
     * @param endTime
     */
    protected Node(String id, SearchState word, int beginTime, int endTime) {
        this.id = id;
        this.state = word;
    ...
    public Word getWord() {
	    ...
	    return ((WordSearchState)state). getPronunciation().getWord();
	else 
	    throw new Error("c'est rate ce n'est pas un word");
    }
	...
	String posterior = "" + getPosterior();
        if (getPosterior() == LogMath.getLogZero()) {
            posterior = "log zero";}
        return ("N"+id+"(" + state + "," + getBeginTime() + "|" + 
	...
	String  label;
	if ( this.state  instanceof WordSearchState) 
	    label=((WordSearchState)state).getPronunciation().toString();
	else
	    {label= ((HMMSearchState)state).getHMMState().getHMM().toString();
	    }

        f.write("node: { title: \"" + id + "\" label: \""
                + label + "[" + getBeginTime() + "," + getEndTime() + 
                " p:" + posterior + "]\" }\n");
    }
	...
	String  label;
	if ( this.state  instanceof WordSearchState) 
	    label=((WordSearchState)state).getPronunciation().getWord().toString();
	else
	    {label="HMM " +  ((HMMSearchState)state).getHMMState().getHMM().getUnit().isFiller() + " "+
		    ((HMMSearchState)state).getHMMState().getHMM().toString();
	    }
