...
public class FBnormal implements Configurable,FB {
	...
	logLikeAudioMax=ps.getFloat("logLikeAudioMax",7000.0f);
    }
	...
	for (int t=0; t< data.length; t++) {
		...
		nextAlpha.put(state, nextAlpha.get(state) +
			      ((HMMStateState)state).getScore(data[t])/lw-logLikeAudioMax);// bizarre mais je ne sais pas faire
		logger.finest("fbn " +state + " sc " + nextAlpha.get(state)+
			      " audio:"+ (((HMMStateState)state).getScore(data[t])/lw)
			      + "  scmod:"+ (((HMMStateState)state).getScore(data[t])/lw-logLikeAudioMax));
	    }
		...
		for (SentenceHMMState state : aph.keySet()){
		    ...
		    if (ordre==1 && logger.isLoggable(Level.FINER)) logger.finer("st :" + state+ " frame" + t + " alpha:" + prob );
	...
	HashMap<SentenceHMMState,Float> oldBeta, nextBeta,alphaCur,alphaPred;
	...
	for (int t=data.length-1;t>=0; t--) {
		...
		if (logger.isLoggable(Level.FINER))
		    logger.finer("s:" + state+ " alpha:" + alphaCur.get(state)+ " beta:" + 
		...
		proba=oldBeta.get(state)+(proba/lw-logLikeAudioMax);
		//on propage 
		...
		for (SearchStateArc arc: state.getPredecessors()) {
			...
			if (noEmit[fromNode.getOrder()].containsKey(fromNode)) 
			    noEmit[fromNode.getOrder()].put(fromNode,
							    logMath.addAsLinear(noEmit[fromNode.getOrder()].get(fromNode),proba+arc.getProbability()));
		    
			else
			    noEmit[fromNode.getOrder()].put(fromNode,proba+arc.getProbability());
		    }
		}
	    }
	...
	saver.save("meanfn",globalMean,globalMixture[0].length);// a virer
	saver.save("varfn",globalVar,globalMixture[0].length);// a virer
