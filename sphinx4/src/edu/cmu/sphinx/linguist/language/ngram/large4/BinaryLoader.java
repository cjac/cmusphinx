 ...
 */
class BinaryLoader {
    private static final String DARPA_LM_HEADER = "Darpa Quadrigram LM";
    private static final int LOG2_BIGRAM_SEGMENT_SIZE_DEFAULT = 9;
    private static final int LOG2_TRIGRAM_SEGMENT_SIZE_DEFAULT = 9;
    ...
    private float[] quadrigramBackoffTable;
  ...
  /**
     * Returns the number of quadrigrams
     * 
     * @return the nubmer of quadrigrams
     */
    public int getNumberQuadrigrams() {
        return numberQuadrigrams;
    }
    /**
 ...
 /**
     * Returns all the quadrigram probabilities.
     * 
     * @return all the quadrigram probabilities
     */
    public float[] getQuadrigramProbabilities() {
        return quadrigramProbTable;
    }

    /**
     * Returns all the quadrigram backoff weights
     * 
     * @return all the quadrigram backoff weights
     */
    public float[] getQuadrigramBackoffWeights() {
        return quadrigramBackoffTable;
    }

    /**
     * Returns the quadrigram segment table.
     * 
     * @return the quadrigram segment table
     */
    public int[] getQuadrigramSegments() {
        return quadrigramSegmentTable;
    }
  ...
  /**
     ...
     * @return the log of the bigram segment size
     */
    public int getLogTrigramSegmentSize() {
        return logTrigramSegmentSize;
    }
   ...
   /**
     * Returns the location (or offset) into the file where quadrigrams start.
     * 
     * @return the location of the quadrigrams
     */
    public int getQuadrigramOffset() {
        return quadrigramOffset;
    }
    ...
    private void loadBinary(File location) throws IOException {
        ...
        skipBigramsTrigramsQuadrigrams(stream);
	...
	if (numberQuadrigrams >0) 
	    {    quadrigramBackoffTable = readFloatTable(stream, bigEndian);
		quadrigramProbTable = readFloatTable(stream, bigEndian);
		int trigramSegmentSize = 1 << logTrigramSegmentSize;
		int quadrigramSegTableSize = ((numberTrigrams + 1) / trigramSegmentSize) + 1;
		quadrigramSegmentTable = readIntTable(stream, bigEndian,
						      quadrigramSegTableSize);
	    }
    ...
    private void readHeader(DataInputStream stream) throws IOException {
        ...
        logTrigramSegmentSize = LOG2_TRIGRAM_SEGMENT_SIZE_DEFAULT;
		...
		logBigramSegmentSize = readInt(stream, bigEndian);
                if (logTrigramSegmentSize < 1 || logTrigramSegmentSize > 15) {
                    throw new Error("log2(tri_seg_sz) outside range 1..15");
                }
            }
  ...
  if ((numberQuadrigrams = readInt(stream, bigEndian)) < 0) {
            throw new Error("Bad number of quadrigrams: " + numberQuadrigrams);
        } else {
            maxNGram = 4;
        }
    }
     ...
     *                the source of data
     */
    private void skipBigramsTrigramsQuadrigrams(DataInputStream stream) throws IOException {
        // skip all the bigram entries, the +1 is the sentinel at the end
        if (numberBigrams > 0) {
            bigramOffset = bytesRead;
            int bytesToSkip = (numberBigrams + 1)
                    * LargeQuadrigramModel.ID_FIELDS_PER_BIGRAM * getBytesPerIDField();
        ...
        if (numberTrigrams > 0) {
            ...
            int bytesToSkip = (numberTrigrams+1) //sentinelle en quadri
                    * LargeQuadrigramModel.ID_FIELDS_PER_TRIGRAM * getBytesPerIDField();
            stream.skipBytes(bytesToSkip);
            bytesRead += bytesToSkip;
        }
	if (numberQuadrigrams > 0) {
            quadrigramOffset = bytesRead;
            int bytesToSkip = (numberQuadrigrams)
                    * LargeQuadrigramModel.ID_FIELDS_PER_QUADRIGRAM * getBytesPerIDField();
            stream.skipBytes(bytesToSkip);
            bytesRead += bytesToSkip;
        }
    }
    ...
    private int[] readIntTable(DataInputStream stream, boolean bigEndian,
        ...
        if (numSegments != tableSize) {
            throw new Error("Bad xgram seg table size: " + numSegments);
        }
