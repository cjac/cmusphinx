/*
 * sen2s2.c -- Mixture density weights associated with each tied state.
 *
 * **********************************************
...
static void senone_dump (const mdef_t *mdef, const senone_t *s, char *file)
{
	...
	for (c = 0; c < fw[f].n_wt; c++) {
		...
		for (i = sb; i <= se; i++) {
		    ...
		    j = s->mgau2sen_idx[i];
		    assert (j == i);
		    fprintf(stderr, "  %4d %4d %4d %4d", f, c, i, fw[f].prob[j][c]);
...
main (int32 argc, char *argv[])
{
    ...
    if (argc != 7)
	E_FATAL("Usage: %s feat mdef mgaumap mixw mixwfloor output\n", argv[0]);
    
    feattype = argv[1];
    mdeffile = argv[2];
    mgaumap = argv[3];
    senfile = argv[4];
    if (sscanf (argv[5], "%lf", &wtflr) != 1)
