...
static void process_ctlfile (FILE *fp)
{
	...
	for (i = 0; i < n_feat; i++) {
	    for (j = 0; j < n_feat; j++) {
		if (dist_type == EUCL_DIST)
		    d = eucl_dist (feat[i], feat[j], featlen);
		else
		    d = maha_dist (feat[i], feat[j], featlen);
		if (maxd < d)
		    maxd = d;
	...
	sq = (7.0/(double)n_feat) * 72.0;	/* Points/unit square */
	...
	printf ("  0 %.2f rlineto\n", sq);
	printf ("  %.2f 0 rlineto\n", -sq);
	printf ("  closepath setgray fill } def\n\n");
	...
	sc = logsc ? 1.0 / log(1.0 + maxd) : 1.0 / maxd;
	
	/* Compute and show distances between every pair of frames */
	for (i = 0; i < n_feat; i++) {
	    for (j = 0; j < n_feat; j++) {
		if (dist_type == EUCL_DIST)
		    d = eucl_dist (feat[i], feat[j], featlen);
		    ...
		    d = maha_dist (feat[i], feat[j], featlen);
		d = logsc ? log(1.0 + d) * sc : d * sc;

		/* Use d or 1.0-d below: */
		printf ("%.3f %.2f %.2f box\n", 1.0-d, i*sq + XOFF, j*sq + YOFF);
	    }
	    printf ("\n");
	}
	...
	for (i = 0; i <= n_feat; i++) {
	    if (i % 10 == 0) {
		printf ("%.2f %.2f hdash\n", n_feat*sq + XOFF+sq, i*sq + YOFF);
		printf ("%.2f %.2f vdash\n", i*sq + XOFF, n_feat*sq + YOFF+sq);
	    } else {
		printf ("%.2f %.2f hdot\n", n_feat*sq + XOFF+sq, i*sq + YOFF);
		printf ("%.2f %.2f vdot\n", i*sq + XOFF, n_feat*sq + YOFF+sq);
	    }
	...
	/* Write phone segmentation if available */
	if (pseg) {
	    sf = 0;

	    for (i = 0; i < n_pseg; i++) {
		printf ("%.2f %.2f hdash\n", n_feat*sq + XOFF+sq*4.0, sf*sq + YOFF);
		printf ("%.2f %.2f vdash\n", sf*sq + XOFF, n_feat*sq + YOFF+sq*4.0);

		if (pseg[i].nf > 2) {
		    printf ("(%s) %.2f %.2f lbl\n", pseg[i].p,
			    n_feat*sq + XOFF + sq*4,
			    (sf + ((pseg[i].nf-2)*0.5))*sq + YOFF);
		    printf ("(%s) %.2f %.2f vlbl\n", pseg[i].p,
			    (sf + ((pseg[i].nf+2)*0.5))*sq + XOFF,
			    n_feat*sq + YOFF + sq*4);
		}
	...
	printf ("(%s) %.2f %.2f lbl\n\n", pslabel, XOFF + n_feat*sq*0.5, YOFF - 16.0);
