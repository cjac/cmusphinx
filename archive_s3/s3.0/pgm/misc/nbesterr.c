/*
 * nbesterr.c -- N-best list oracle error rate (using DP alignment).
 *
 * **********************************************
...
static dag_t *hyp_load (char *line)
{
    dag_t *dag;
    char wd[4096], *lp;
    int32 len, scr, n, sf, k;
    s3wid_t wid;
    dagnode_t *d, *prevd;
    
    /* Read data */
    lp = line;
    if (sscanf (line, "T %d A %d L %d%n", &scr, &scr, &scr, &len) != 3) {
    ...
    if (sscanf (lp, "%d %d %d %s%n", &sf, &scr, &scr, wd, &len) != 4)
	E_FATAL("Bad line: %s\n", lp);
    lp += len;
    ...
    n = 0;
    while ((k = sscanf (lp, "%d %d %d %s%n", &sf, &scr, &scr, wd, &len)) == 4) {
	lp += len;
...
static dag_t *nbest_load (char *file, char *uttid, dagnode_t *ref, int32 nref)
{
    FILE *fp;
    dag_t *dag;
    char line[16384], wd[4096], *lp;
    char bestline[16384];
    int32 len, scr, n, sf, k, best, nhyp;
    s3wid_t wid;
    dagnode_t *d, *prevd;
    dpnode_t retval;
	...
	if (dag) {
	    ...
	    retval = dp (uttid, dict, oovbegin, ref, nref, dag, &nhyp, 0, 0);
	    dag_destroy (dag);
...
static void process_reffile (char *reffile)
{
	...
	dag = nbest_load (file, uttid, ref, nref);
	if (! dag) {
