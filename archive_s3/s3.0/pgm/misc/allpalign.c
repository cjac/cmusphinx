...
{
    FILE *rfp;
    char *allpdir, allpfile[4096], line[32768], uttid[4096];
    int32 nref, nhyp;
	...
	i = strlen(line);
	if (line[i-1] != '\n')
	    E_FATAL("Line not completely read: %s\n", line);
	...
	retval = dp (uttid, dict, dict->n_word, ref, nref, dag, &nhyp, 1, 1);
...
static arg_t arglist[] = {
    ...
    { "-dict",
      CMD_LN_STRING,
      NULL,
      "Dictionary input file with phonemes as words (pronunciations not relevant)" },
    { "-fdict",
      CMD_LN_STRING,
      NULL,
      "Dictionary input file with noise phonemes as words (pronunciations not relevant)" },
    { "-ref",
      CMD_LN_STRING,
      NULL,
      "Time-segmented reference input file (phoneme segments from forced alignment)" },
    { "-allp",
      CMD_LN_STRING,
      ".",
...
main (int32 argc, char *argv[])
{
    ...
    if (NOT_WID(silwid))
	E_FATAL("<sil> not defined in dictionary\n");
