...
void vector_print(FILE *fp, vector_t v, uint32 dim)
{
    int32 i;
...
/* Like vector_dist_eucl, but skip every odd-indexed dimension */
static float64 vector_dist_eucl2 (float32 *v1, float32 *v2, int32 len)
{
    float64 d;
    int32 i;
...
/* Like vector_vqlabel, but skip every odd-indexed dimension in computing vector distances */
static int32 vector_vqlabel2 (float32 *vec, float32 **mean, int32 rows, int32 cols,
			      float64 *sqerr)
{
    int32 i, besti;
    float64 d, bestd;
    ...
    bestd = vector_dist_eucl2 (mean[0], vec, cols);
    besti = 0;
    
    for (i = 1; i < rows; i++) {
	d = vector_dist_eucl2 (mean[i], vec, cols);
	if (bestd > d) {
	    bestd = d;
	    besti = i;
	}
		      ...
		      float32 **mean, int32 *map)
{
    ...
    float64 eps, sqerr, prev_sqerr, t;
    bitvec_t sel;
    int32 *count;
    float32 *gmean;
    ptmr_t tm;
...
float64 vector_vqgen2 (float32 **data, int32 rows, int32 cols, int32 vqrows,
		       float64 epsilon, int32 maxiter, float32 **mean, int32 *map)
{
	...
	sqerr = 0.0;
	for (i = 0; i < rows; i++) {
	    map[i] = vector_vqlabel2 (data[i], mean, vqrows, cols, &t);
	    sqerr += t;
	}
	ptmr_stop(&tm);
...
void vector_gautbl_alloc (vector_gautbl_t *gautbl, int32 n_gau, int32 veclen)
{
    ...
    gautbl->distfloor = logs3_to_log (LOGPROB_ZERO);
}
