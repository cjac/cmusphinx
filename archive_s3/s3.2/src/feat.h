...
feat_t *feat_init (char *type,		/* In: Type of feature stream */
		   char *cmn,		/* In: Type of cepstram mean normalization to be done
					   before feature computation; can be NULL (for none) */
		   char *varnorm,	/* In: ("yes" or "no") Whether variance normalization
					   done on each utt; only applicable if CMN also done */
		   char *agc);		/* In: Type of automatic gain control to be done
					   before feature computation; can be NULL (for none) */
...
/*
 * Read a specified MFC file (or given segment within it), perform CMN/AGC as indicated by
 * fcb, and compute feature vectors.  Feature vectors are computed for the entire segment
 * specified, by including additional surrounding or padding frames to accommodate the feature
 * windows.
 * Return value: #Frames of feature vectors computed if successful; -1 if any error.
 */
int32 feat_s2mfc2feat (feat_t *fcb,	/* In: Descriptor from feat_init() */
		       char *file,	/* In: File to be read */
		       char *dir,	/* In: Directory prefix for file, if needed;
					   can be NULL */
		       int32 sf, int32 ef,	/* Start/End frames within file to be read.
						   Use 0,-1 to process entire file */
		       float32 ***feat,	/* Out: Computed feature vectors; caller must allocate
					   this space */
		       int32 maxfr);	/* In: Available space (#frames) in above feat array;
					   it must be sufficient to accommodate the result */
