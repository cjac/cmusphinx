 ...
 */
int32 s2mfc_read (char *file, int32 sf, int32 ef, float32 **mfc, int32 maxfr)
{
    FILE *fp;
    int32 n_float32;
    struct stat statbuf;
...
main (int32 argc, char *argv[])
{
    ...
    if (((argc != 3) && (argc != 5)) ||
	((argc == 5) && (sscanf (argv[3], "%d", &sf) != 1)) ||
	((argc == 5) && (sscanf (argv[4], "%d", &ef) != 1)) ||
	(ef <= sf) || (sf < 0) || (ef <= 0)) {
	E_INFO("Usage: %s FeatType MFCfile [StartFrame EndFrame]\n", argv[0]);
	exit(0);
    }
    
    fcb = feat_init (argv[1]);
    if (! fcb->compute_feat)
	E_FATAL("Not testing generic feature type\n");
    
    mfc = (float32 **) ckd_calloc_2d (S3_MAX_FRAMES, feat_cepsize(fcb), sizeof(float32));
    fv = feat_vector_alloc (fcb);
    
    nfr = s2mfc_read (argv[2], sf, ef, mfc, S3_MAX_FRAMES);
    if (nfr <= 0)
	E_FATAL("s2mfc_read(%s) failed\n", argv[2]);
    else
	E_INFO("%s: %d frames\n", argv[2], nfr);
    
    for (f = feat_window_size(fcb); f < nfr - feat_window_size(fcb); f++) {
	fcb->compute_feat (fcb, mfc+f, fv);
	feat_print (fcb, &fv, 1, stdout);
    }
}
#endif
