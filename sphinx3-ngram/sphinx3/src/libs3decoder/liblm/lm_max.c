				...
				)
{
    ...
    for (i = 0; i < lm_max->n_bg; i++) {
		...
		lm_max->bg_table[i].max_bowt.l
			= logmath_log10_to_log(logmath, lm_max->bg_table[i].max_bowt.f);
	}

    /* trigrams */
	for (i = 0; i < lm_max->n_tg; i++)
		lm_max->tg_table[i].max_prob.l
			= logmath_log10_to_log(logmath, lm_max->tg_table[i].max_prob.f);
}
...
lm_max_t * lm_max_read_dump (const char * filename,
							 int32 applyweight, float64 lw, float64 wip, logmath_t *logmath)
{
	...
	if (lm_max != NULL) {
		...
		if (applyweight) {
			...
			lmmax2logs3 (lm_max, logmath);

			/* apply the new lw and wip values */
			lm_max_set_param (lm_max, lw, wip, logmath);
		}
	}
