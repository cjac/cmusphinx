...
void Usage() {
    cout << "\nUSAGE:\n";
    cout << "consensus -f -o -s  -i <infile> [-c <outfile>] [-C <outdir>] -R <pronfile> [-G <logfile>] [-S <scale>] [-L <LMweight>] [-l <PMweight>] " << endl 
         << " [-P <PMweight>] [-p <PMoldscale>] [-I <WIP>] [-D <DELweight>] [-T <thresh>] [-t <percentage>] [-m <intraword>] [-M <interword>]  " << endl
         << " [-e <lowEpsT>} [-E <highEpsT>] [-w <lowWordT>] [-W <highWordT>] " << endl << endl
	 << "where " << endl
	 << "-f               - specifies that the format of the input file is FSM" << endl               
...
int main(int argc, char * argv[]) {
    GetOpt opt(argc, argv, "fosnbi:c:C:R:S:G:L:l:P:p:I:E:e:W:w:D:T:t:m:M:x:");
    int ch;
    ...
    if (pronfile == ""){
	cout << "Please provide the name of the file containing the no.of pronunciations and the most likely pronunciation for each word:" << endl;
	getline(cin, pronfile);
	assert(pronfile != "");
    }	
    Prons P(pronfile);
	...
	cout << "Lattice from " << file_name << endl;
	 
	// usually the files are compressed, but in the list of lattices they don't have the extension .gz
	...
	if (lat.Type() == "SLF"){
	    ...
	    lat.compute_link_scores(LMweight, PRweight, WIP, scale);  // compute link probabilities
	}
	...
	cout << "No of Nodes: " << lat.no_nodes() << endl;
	cout << "No of Links: " << lat.no_links() << endl;
	cout << "No of Words: " << lat.no_words() << endl;
	...
	cout << "No of Clusters: " << C.no_clusters() << endl;
        int no_links_left = C.No_links();
        float x = (no_links_left * 100)/lat.no_links();
        int no_words_left = C.no_words();
        float y = (no_words_left * 100)/lat.no_words();

        cout << "No of Links   : " << no_links_left << "\t(" << x << "% of the original) " << endl;
        cout << "No of Words   : " << no_words_left << "\t(" << y << "% of the original) " << endl << endl;
	...
	if (method_intraword == "avg")
        ...
        cout << endl << endl;
        cout << "\t\t\t\tMerging steps in INTRA-WORD CLUSTERING"  << endl;
        cout << endl;
	...
	if (logfile != "" && first){
        ...
        cout << endl << endl;
        cout << "\t\t\t\tClusters after INTRA-WORD CLUSTERING\n";
        cout << endl;
	...
	cout << endl << endl;
        cout << "\t\t\t\tMerging steps in INTER-WORD CLUSTERING" << endl;
        cout << "No of clusters: " << C.no_clusters() << endl << endl;
	...
	cout << endl << endl;
        cout << "\t\t\t\tClusters after INTER-WORD CLUSTERING\n";
