...
void Usage() {
    cout << "\nUSAGE:\n";
    cout << "consensus  -f -o -s -i <filelist> [-c <outfile>] [-C <outdir>] -R <pronfile> [-G <logfile>] [-S <scale>] [-L <LMweight>] [-l <PMweight>] " << endl 
...
int main(int argc, char * argv[]) {
	...
	C.print_sausages_FSMformat(outdir, outfile, file_name, lowEpsT, highEpsT, lowWordT, highWordT, P, output_fullpath); 
	first = false;
    }
    flist.close();
    return 0;
