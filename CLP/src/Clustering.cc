...
Clustering::go_cluster(const Prons& P, const int stage, const int mode, const bool constrained)
{
  ...
  vector<ClusterListIt> most_similar(no_clusters); // for each cluster keep a pointer to the most similar cluster to it
  ...
  // add in SIM the similarity values between the compatible clusters  
	...
	if (id1 < id2){
	  ...
	  else if (stage == 4){      // the last stage that ensures a total order
	    if (SIM[id1][id2] == 0 && SIM[id2][id1] == 0){
    ...
    vector<unsigned> eliminated(no_clusters);   // keeps track of the clusters that have been eliminated after merging
...
Clustering::compute_PHON_sim(ClusterListConstIt cit1, ClusterListConstIt cit2, Prons& P, IntIntDblMap& wordPhSim, const int mode, bool use_phon_info, bool constrained)
{
	...
	if (use_phon_info){
	  ...
	  if (it == wordPhSim.end()){
	    ...
	    const string& pron1 = P.get_pron((*it1).first); // MOD 7/3/2000 - const added
	    const string& pron2 = P.get_pron((*it2).first); // MOD 7/3/2000 - const added
	    if (pron1 == "" || pron2 == "" )
