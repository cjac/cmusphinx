			...
			int32 iInFeatLen)
{
	...
	if ((self->iFeatVectLen != iInFeatLen) || (self->iFeatVectLen != iOutFeatLen))
	{
		E_FATAL("Feature vector length mismatch");
	}
	...
	if (self->iFrameCount == 0)
	{
		...
		{
			//	self->pdOnlineShortMean[i]   = pInFeat[i];//DEFAULT_MEAN[i];
			self->pdOnlineMean[i]        = pInFeat[i];//DEFAULT_MEAN[i];
			//	self->pdOnlineLongMean[i]    = pInFeat[i];//DEFAULT_MEAN[i];
	...
	if (self->iBufferSize > 0)
	{
		...
		if (self->iFrameCount < self->iBufferSize)
		{
			...
			//memcpy(self->ppfCepBuffer[i], pInFeat, sizeof(float32) * iInFeatLen);

			memcpy(self->ppfCepBuffer[self->iFrameCount], pInFeat, sizeof(float32) * iInFeatLen);
		...
		else
		{
			...
			{
				memcpy(self->ppfCepBuffer[i], self->ppfCepBuffer[i + 1], sizeof(float32) * iInFeatLen);
			}

			memcpy(self->ppfCepBuffer[self->iBufferSize - 1], pInFeat, sizeof(float32) * iInFeatLen);
		}
	}
	/////////////////
	//
	...
	if (self->iFrameCount == 0)
	{
		FILE* pFile = fopen("VADResult.txt", "wt");
	...
	if (self->iFrameCount >= self->iBufferSize)
	{
		...
		for (i = 0; i < self->iFeatVectLen; i++)
		{
			self->pdOnlineMean[i] = self->dLongLamda * self->pdOnlineMean[i] + (1 - self->dLongLamda) * pInFeat[i];
			pOutFeat[i]       = pInFeat[i] - self->pdOnlineMean[i];
		}
		...
		if (self->iFrameCount == 0)
		{
			...
			//	self->pdOnlineMinusMean[0]   = pOutFeat[0];
			self->pdOnlinePlusMean[0]    = pOutFeat[0];
			self->pdOnlineShortMean[0]   = pOutFeat[0];//DEFAULT_MEAN[i];
			self->pdOnlineLongMean[0]    = pOutFeat[0];//DEFAULT_MEAN[i];
		...
		for (i = 0; i < self->iFeatVectLen; i++)
		{
			self->pdOnlineShortMean[i] = self->dShortLamda* self->pdOnlineShortMean[i] + (1 - self->dShortLamda) *  pOutFeat[i];
			self->pdOnlineLongMean[i]  = self->dLongLamda * self->pdOnlineLongMean[i]  + (1 - self->dLongLamda)  *  pOutFeat[i];
		}
		...
		else
		{
			...
			if (pOutFeat[0] < self->pdOnlineMinusMean[0])
			{
				/////////////////////////////////////////////////////////////////////
				//
		...
		{
			FILE* pFile = fopen("VADResult.txt", "at");


			fprintf(pFile, "%f %f %f %f %f  %d %d\n", self->pdOnlineShortMean[0],
				self->pdOnlineMean[0],
				self->pdOnlineMinusMean[0],
				self->pdOnlinePlusMean[0],
				self->pdOnlineLongMean[0], 
				self->VadState,
				self->iSilSkip);
		...
		for (i = 0; i < self->iFeatVectLen; i++)
		{
			if (self->VadState == SPEECH)
			{
				//	self->pdShortVar[i]      = (1 - self->dLongLamda) * self->pdOnlineVar[i] + (1 - self->dLongLamda) * SQ(pOutFeat[i]);
				self->pdOnlineVar[i]     = (self->dLamda) * self->pdOnlineVar[i] + (1 - self->dLamda) * SQ(pOutFeat[i]);
			}
			...
			if (self->iFrameCount >= self->iBufferSize)
			{
				if (self->iBufferSize > 0)
				{
					/* FIXME: Chanwoo's error here, was adStdMFCC? */
					pOutFeat[i] =  (float32)( (self->ppfCepBuffer[0][i] - self->pdOnlineMean[i]) / sqrt(self->pdOnlineVar[i]) * ms_adStdPCC[i] * 2 );
				}
				else
				{
					pOutFeat[i] =  (float32)((pInFeat[i] - pOutFeat[i]) / sqrt(self->pdOnlineVar[i]) * ms_adStdPCC[i]) * 0.2;
				}
			}
