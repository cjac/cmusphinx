...
# Compile
my $compilation = system("cd $s3root; autogen.sh && autogen.sh && make");
...
my job_id = "";
if ($dependence eq "") {
  system("perl regression_report.pl $localregression_log $test_file");
} else {
  ...
  print REGRESSION "perl regression_report.pl $localregression_log $test_file");
  close(REGRESSION);
