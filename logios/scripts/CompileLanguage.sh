...
here=$PWD
if [ -z "$LOGIOS_ROOT" ]; then LOGIOS_ROOT=$here
...
if [ -n "$OLYMPUS" ]; then
    ...
    perl $LOGIOS_ROOT/Tools/MakeLanguage/make_language.pl --logios $LOGIOS_ROOT --olympus \
	--resources $RESOURCES \
...
#   :: otherwise --inpath and --outpath give you control over actual locations
    ...
    perl $LOGIOS_ROOT/Tools/MakeLanguage/make_language.pl --logios $LOGIOS_ROOT \
